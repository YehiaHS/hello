...
const SectionShell = ({ id, label, title, children }) => (
  <section
    ...
    className="group relative border-4 border-ink bg-white p-8 shadow-brutal transition-all hover:-translate-y-2 hover:shadow-brutalLg"
  >
...
const Hero = () => {
  ...
  return (
    ...
    <header className="relative border-4 border-ink bg-white p-10 shadow-brutal md:p-20 overflow-hidden group">
      <div className="absolute -right-20 top-20 h-64 w-64 rotate-12 border-4 border-ink bg-brutalYellow opacity-100 shadow-brutal" />
      <div className="absolute -left-20 bottom-20 h-48 w-48 -rotate-12 border-4 border-ink bg-brutalCyan opacity-100 shadow-brutal" />

      <div className="relative z-10 grid gap-12 md:grid-cols-[1.5fr_1fr]">
        <div className="space-y-8">
          <div className="inline-block border-4 border-ink bg-brutalLime px-6 py-2 font-mono text-sm uppercase tracking-widest shadow-brutalSm font-bold transform -rotate-2">
            {t('portfolio')}
          </div>

          <div className="space-y-2">
            <div className="text-3xl font-bold text-ink md:text-4xl font-mono">
              {t('hello')}
            </div>
            <h1 className="text-7xl font-black leading-[0.9] md:text-9xl tracking-tighter mix-blend-hard-light hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-brutalMagenta hover:to-brutalYellow transition-all duration-300 cursor-default">
              {profile.name}
            </h1>
          </div>

          <p className="max-w-xl text-2xl font-bold text-ink border-l-8 border-brutalMagenta pl-6 py-2 bg-alabaster">
            {t('tagline')}
          </p>

          <p className="max-w-2xl text-lg font-mono leading-relaxed bg-white border-2 border-ink p-4 shadow-brutalSm">
            {profile.summary}
          </p>
        ...
        <div className="relative flex flex-col gap-6 border-4 border-ink bg-ink p-8 text-alabaster shadow-brutalReverse">
...
const BrutalistTicker = () => {
  ...
  return (
    <div className="relative overflow-hidden border-y-4 border-ink bg-ink text-brutalLime py-6 rotate-1 my-10 shadow-brutal">
      <div className="flex whitespace-nowrap">
        <div className="animate-ticker flex min-w-[200%] items-center gap-16">
    ...
    <SectionShell id="skills" label="Capabilities" title={t('skillsTitle')}>
      <div className="grid gap-8 md:grid-cols-2">
        {profile.skills.map(({ title, items, accent }) => (
          <div key={title} className="flex h-full flex-col gap-6 border-4 border-ink bg-white p-8 shadow-brutalSm hover:bg-ink hover:text-white transition-colors group">
    ...
    <SectionShell id="education" label="Education" title={t('educationTitle')}>
      <div className="space-y-8">
        {profile.education.map((item) => (
          <div key={item.degree} className="relative border-4 border-ink bg-white p-8 shadow-brutal hover:bg-ink hover:text-white transition-colors group">
  ...
  return (
    <div className="min-h-screen bg-alabaster pb-24">
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-20 px-4 pt-20 md:gap-24 md:px-10">
        ...
        >
          <div className="flex items-center justify-between border-2 border-ink bg-brutalYellow px-10 py-8 transition-colors group-hover:bg-brutalLime">
            ...
            <span className="text-6xl font-black transition-transform group-hover:translate-x-4">→</span>
          </div>
        </Link>
